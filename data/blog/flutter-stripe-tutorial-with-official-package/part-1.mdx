...
<header>
  ...
  interesting mobile app built with Flutter. This is sort of a rendition of my old [Flutter Stripe
  Tutorial
  Series](https://www.youtube.com/watch?v=VOtmM9i25R4&list=PL2sQdFoGnLIgvo5DviCeCPZnfeVbCrRsV). The
  package I used in that one although is deprecated now. Sad lyf!
</header>
...
We have a repository already set up. In order to follow this tutorial, you need to clone the repository and then checkout the `start_here` branch.
...
<ImageWithBg
  ...
  src="/static/images/flutter-stripe-tutorial/copy-api-keys.png"
  caption="Figure 1.2"
  backgroundColor="transparent"
/>

## Install the flutter_stripe package

Open the cloned repository in android studio, and add the the latest version of the `flutter_stripe` package inside the `pubspec.yaml` in the `dependencies`. It should look something as follows:
...
## Creating the backend for the stripe integration

We're going to use [Firebase Functions](https://firebase.google.com/docs/functions) (or "Cloud functions for Firebase" as they call it now) as the backend for our stripe integration.
You can however a simple NodeJS/ExpressJS app to do the same.

First, make sure you have [NodeJS](https://nodejs.org/en/) and
[Git](https://git-scm.com/downloads) installed in your system.
Essentially you need to create a Firebase Project first in the [Firebase Console](https://console.firebase.google.com).
Then you need to create firebase functions inside this repository by following the [Firebase Functions getting started guide](https://firebase.google.com/docs/functions/get-started).
Essentially, you need to run the following commands in the terminal, making sure you're in the project directory:
...
# creating a firebase functions folder
firebase init functions
```

You should see the following prompt. Choose 'Use an existing project', and then select the project that you created on the [Firebase Console](https://console.firebase.google.com).

<ImageWithBg
  title="Create firebase functions"
  alt="Create firebase functions image"
  src="/static/images/flutter-stripe-tutorial-with-official-package/firebase-functions-init.png"
  caption="Figure 1.7"
  backgroundColor="transparent"
/>

Make sure to select **Typescript** as the language for your functions and enabling **ESLint**. Then, proceed forward with the installation.

<ImageWithBg
  title="Firebase functions setup"
  alt="Firebase functions setup image"
  src="/static/images/flutter-stripe-tutorial-with-official-package/ff-setup.png"
  caption="Figure 1.8"
  backgroundColor="transparent"
/>

Now, create a new file inside the `functions` folder called `.env.local`. And add the following lines:

```bash
# Stripe API keys - see https://stripe.com/docs/development/quickstart#api-keys
STRIPE_PUBLISHABLE_KEY=pk_test_xxxx # Replace with your publishable key
STRIPE_SECRET_KEY=sk_test_xxx # Replace with your secret key
```

Now, we're going to set up the server itself. To do that, navigate to the `functions` folder using your terminal and add the following npm packages by running the command:
...
npm install --save body-parser express dotenv stripe
```

Once the dependencies are installed, open the `functions/src/index.ts` file and update it with the following lines of code:
...
Notice that the above snippet makes use of the `utils` file. That doesn't exist so far. Let's create a new file named `utils.ts` inside the `functions/src` folder.
