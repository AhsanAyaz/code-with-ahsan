...
import { SESSION_TEMPLATES, SessionTemplate, generateGoogleCalendarUrl } from '@/lib/mentorship-templates'
...
export default function SessionScheduler({ matchId, currentUserId, isMentor, menteeEmail }: SessionSchedulerProps) {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const response = await fetch(`/api/mentorship/scheduled-sessions?sessionId=${matchId}`)
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setSessions(prev => [...prev, data.scheduledSession].sort((a, b) => 
          new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime()
        ))
        setFormData({ date: '', time: '', duration: 30, agenda: '', templateId: '' })
  ...
  const openGoogleCalendar = (session: Session) => {
    ...
    const title = template ? `Mentorship: ${template.title}` : 'Mentorship Session'
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-base-content/60">Plan and track your mentorship meetings</p>
        </div>
        {isMentor && (
          <button 
        ...
        <div className="card bg-base-200 p-4 space-y-4">
          ...
          <div>
            <label className="label">
              <span className="label-text font-semibold">Choose a Template (Optional)</span>
            </label>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">Duration: {formData.duration} mins</span>
              </label>
              <input
                type="range"
                min={15}
      ...
      {!isMentor && upcomingSessions.length === 0 && (
        <div className="alert alert-info">
          <span>Your mentor will schedule sessions. You&apos;ll see them here once scheduled.</span>
        </div>
      )}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <div className="font-semibold flex items-center gap-2">
                          ...
                          <span className="badge badge-ghost badge-sm">{session.duration} min</span>
                        </div>
