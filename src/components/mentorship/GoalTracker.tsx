...
export default function GoalTracker({ matchId, currentUserId, isMentor }: GoalTrackerProps) {
  ...
  useEffect(() => {
    const fetchGoals = async () => {
      try {
        const response = await fetch(`/api/mentorship/goals?matchId=${matchId}`)
        if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-base-content/60">Track your mentorship objectives</p>
        </div>
        <button 
                ...
                <div className="card-body p-3">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={() => toggleGoalStatus(goal.id, goal.status)}
                    />
                    <span className="line-through text-base-content/50">{goal.title}</span>
                  </div>
                </div>
              </div>
            ))}
