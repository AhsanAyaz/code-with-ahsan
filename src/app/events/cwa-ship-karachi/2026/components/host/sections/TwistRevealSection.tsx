...
export default function TwistRevealSection({ twistPhase, onRevealed }: TwistRevealSectionProps) {
  const [count, setCount] = useState(5);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Re-arm the counter as the phase flips, during render rather than in an
  // effect. https://react.dev/learn/you-might-not-need-an-effect
  const [renderedPhase, setRenderedPhase] = useState(twistPhase);
  if (renderedPhase !== twistPhase) {
    setRenderedPhase(twistPhase);
    if (twistPhase === "countdown") setCount(5);
  }
