...
const features: { icon: LucideIcon; title: string; description: string; color: string }[] = [
  {
    ...
    title: "Collaboration",
    description:
      "Experts and beginners coding side-by-side. Cultivating the next generation of engineers.",
    color: "primary",
  },
  {
    icon: GraduationCap,
    title: "Mentorship",
    description:
      "Mentors guiding teams, providing real-time code reviews and architectural advice.",
    color: "accent",
  },
  {
    icon: Rocket,
...
const FeatureCard = ({ feature, index }: { feature: (typeof features)[0]; index: number }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg sm:text-xl font-semibold text-base-content mb-3">{feature.title}</h3>
      <p className="text-base-content/70 text-sm leading-relaxed">{feature.description}</p>

      {/* Bottom corner decoration */}
      <div
        className={`absolute bottom-0 right-0 w-12 h-12 ${isPrimary ? "border-primary/10" : "border-accent/10"} border-b border-r rounded-br-2xl opacity-0 group-hover:opacity-100 transition-opacity`}
      />
    </motion.div>
  );
};
