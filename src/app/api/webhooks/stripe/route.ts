    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        try {
          ...
          logger.warn("Google Calendar event creation bypassed/failed", { calErr });
        }

        // If no Meet link was generated by Google Calendar, provide a standard fallback
        if (!meetLink) {
          meetLink = `https://meet.google.com/lookup/cwa-consulting-${bookingId.slice(0, 8)}`;
        }

        // Update Firestore booking record safely (no undefined values)
        const now = new Date();
        ...
        try {
          ...
          logger.error("Non-fatal email dispatch error in webhook", { emailErr });
        }
        break;
      }
