...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [activeSessionsSnapshot, completedSessionsSnapshot, ratingsSnapshot] = await Promise.all([
      db.collection('mentorship_sessions').where('status', '==', 'active').get(),
    ...
    completedSessionsSnapshot.docs.forEach(doc => {
      const mentorId = doc.data().mentorId
      mentorCompletedCounts[mentorId] = (mentorCompletedCounts[mentorId] || 0) + 1
    })
    ...
    if (publicOnly) {
      mentors = mentors.filter(m => 
        m.isPublic !== false && m.status === 'accepted'
      )
    }
