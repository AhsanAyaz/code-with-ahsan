...
import { useMentorship, MentorshipMatch, MentorshipProfile } from '@/contexts/MentorshipContext'
      ...
      {pendingMatches.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
            ...
            {profile.role === 'mentee' ? 'Awaiting Response' : 'Your Pending Approvals'}
          </h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pendingMatches.map(match => (
              <div key={match.id} className="card bg-base-100 shadow-lg border-2 border-warning/20">
                <div className="card-body p-4">
                  <div className="flex items-center gap-4">
                    <div className="avatar">
                      <div className="w-12 h-12 rounded-full">
                        {match.partnerProfile?.photoURL ? (
                          <img src={match.partnerProfile.photoURL} alt={match.partnerProfile.displayName || 'User'} />
                        ) : (
                    ...
                    <div>
                      <div className="font-semibold">{match.partnerProfile?.displayName || 'Unknown'}</div>
        ...
        {loadingMatches ? (
          ...
          <div className="card bg-base-100 shadow-xl">
            <div className="card-body text-center py-12">
              ...
              <h3 className="text-xl font-semibold">No active connections yet</h3>
              <p className="text-base-content/70">
                {profile.role === 'mentee' 
                ...
                <div className="card-body">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-lg">{match.partnerProfile?.displayName}</h4>
                      <p className="text-sm text-base-content/70">
                        {match.partnerProfile?.currentRole || match.partnerProfile?.education || ''}
                      </p>
                      <div className="text-xs text-base-content/50 mt-1">
                        Matched on {match.approvedAt ? new Date(match.approvedAt).toLocaleDateString() : 'N/A'}
                      </div>
                    </div>
                  </div>

                  {/* Expertise/Skills Tags */}
                  {(match.partnerProfile?.expertise || match.partnerProfile?.skillsSought) && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      {(match.partnerProfile?.expertise || match.partnerProfile?.skillsSought || []).slice(0, 4).map(tag => (
