...
export default function GoalsPage() {
  ...
  useEffect(() => {
    const fetchGoals = async () => {
      ...
      try {
        const response = await fetch(`/api/mentorship/all-goals?uid=${user.uid}&role=${profile.role}`)
        if (response.ok) {
      ...
      <div className="stats shadow w-full bg-base-100">
        ...
        <div className="stat">
          ...
          <div className="stat-value text-info">
            {allGoals.length > 0 ? Math.round((completedCount / allGoals.length) * 100) : 0}%
          </div>
        </div>
      </div>
        ...
        <div className="card bg-base-100 shadow-xl">
          <div className="card-body text-center py-12">
            ...
            <p className="text-base-content/70">
              Connect with a {profile.role === 'mentor' ? 'mentee' : 'mentor'} and start setting goals together!
            </p>
          </div>
        </div>
      ) : (
                    ...
                    {match.goals.map(goal => (
                      <div 
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg ${
                          goal.status === 'completed' ? 'bg-success/10' : 'bg-base-200'
                        }`}
                      >
                        <span className={`text-lg ${goal.status === 'completed' ? 'text-success' : ''}`}>
                          {goal.status === 'completed' ? '✅' : '⏳'}
                        </span>
                        <span className={goal.status === 'completed' ? 'line-through text-base-content/60' : ''}>
                          {goal.title}
                        </span>
                        {goal.targetDate && (
