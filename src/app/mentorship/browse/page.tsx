...
export default function BrowseMentorsPage() {
  ...
  useEffect(() => {
    const fetchMyRequests = async () => {
      if (!user) return
      try {
        const response = await fetch(`/api/mentorship/mentee-requests?menteeId=${user.uid}`)
        if (response.ok) {
          const data = await response.json()
  ...
  const handleRequestMatch = async (mentorId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMentorRequestStatus(prev => new Map(prev).set(mentorId, 'pending'))
      } else {
        const error = await response.json()
        if (response.status === 409) {
          // Already requested - update with actual status
          setMentorRequestStatus(prev => new Map(prev).set(mentorId, error.status || 'pending'))
        } else {
