...
import { getFirestore, query, collection, getCountFromServer, where } from 'firebase/firestore'
        ...
        <dl className="flex flex-col my-4 gap-4 items-center">
          {course.duration && (
            <div className="flex items-center gap-4">
              <dt className="text-sm font-medium text-base-content/70">Course Duration</dt>
              <dd className="text-xs text-base-content/60">{course.duration}</dd>
            </div>
          )}
        </dl>
        ...
        <section>
          ...
          <Link
            ...
            className={`flex items-center gap-4 justify-between px-4 py-2 backdrop-blur border border-primary-500/20 dark:border-primary-500/30 hover:border-primary-500/40 dark:hover:border-primary-500/50 transition-colors rounded-lg shadow-lg cursor-pointer ${
              course.active
                ? 'bg-primary-500 text-primary-content dark:bg-primary-800'
                : 'bg-gray-200 dark:bg-gray-800/50'
            }`}
          >
            <span className="break-words text-base-content hover:text-primary-600 dark:hover:text-primary-300">
              View Submissions
            </span>
          </Link>
        </section>
