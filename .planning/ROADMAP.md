...
- [x] 02-04-cohort-api-admin-panel-PLAN.md — `/api/ambassador/cohorts` GET/POST + `[cohortId]` GET/PATCH + `/admin/ambassadors/cohorts` panel (create / toggle window / view attached ambassadors) (Wave 2 / COHORT-01/02/03)
- [x] 02-05-applications-submit-api-PLAN.md — POST submit + GET admin list + GET `/me` + POST `student-id-upload-url` (signed PUT); age gate + cohort window + duplicate guard + DISC-01 fail-soft lookup + EMAIL-01 trigger (Wave 2 / APPLY-01..08, DISC-01, EMAIL-01)
...
- [x] 09-01-PLAN.md — Shared bracketed-mono eyebrow helper + reuse-forced edits (PortfolioBio h1→h2 prop, TestimonialsSection empty-state guard) [wave 1]
- [x] 09-02-PLAN.md — Persistent header Sponsor button in LayoutWrapper navbar-end (teal, D-05) [wave 1]
- [x] 09-03-PLAN.md — New home components: rebuilt CommunityHero (proof manifest + 3 CTAs), TrustedByStrip, teal SponsorBand [wave 2]
- [x] 09-04-PLAN.md — Sponsors enrichment: About-Ahsan (PortfolioBio) + work showcase, async conversion for getCourses [wave 2]
- [x] 09-05-PLAN.md — Home recompose (page.tsx locked section order) + quality-floor build verification + human visual QA [wave 3]
