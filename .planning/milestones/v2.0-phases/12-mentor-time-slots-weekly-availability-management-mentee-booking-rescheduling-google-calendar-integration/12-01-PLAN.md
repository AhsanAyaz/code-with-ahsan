...
- Return `{ availability: TimeSlotAvailability | null, unavailableDates: UnavailableDate[], googleCalendarConnected: boolean }` from the profile
- If mentor has no availability set, return `{ availability: null, unavailableDates: [], googleCalendarConnected: false }`
...
- The `googleCalendarConnected` field is read from the `googleCalendarConnected` boolean field on the mentorship_profiles document (set to `true` by the OAuth callback in Plan 12-03). Default to `false` if not present: `googleCalendarConnected: !!profileData.googleCalendarConnected`
