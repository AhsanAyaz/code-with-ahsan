...
must_haves:
  truths:
    ...
    - "Mentor can cancel a booking and mentee receives Discord DM notification (reschedule-aware: if mentor cancels with reason containing 'reschedule', DM prompts mentee to rebook)"
...
- Return bookings with profile data included, explicitly including these fields in each booking object: `id, mentorId, menteeId, mentorProfile, menteeProfile, startTime, endTime, timezone, status, calendarEventId, calendarSyncStatus, cancelledBy, cancelledAt, cancellationReason, createdAt, updatedAt`
...
- **Send Discord DM notification** (non-blocking, following existing pattern):
  ...
  - **Reschedule-aware message format:** When the mentor cancels, check if `reason` contains "reschedule" (case-insensitive). Use a differentiated message:
    ```typescript
    ...
    const message = isRescheduleRequest
      ? `**Reschedule Requested**\n\nYour mentor ${cancellerName} has requested to reschedule your mentorship session on ${formattedDate} at ${formattedTime} (${timezone}).\n${reason ? `Reason: ${reason}\n` : ''}\nPlease visit the platform to book a new time: https://codewithahsan.dev/mentorship/book/${bookingData.mentorId}`
      : `**Session Cancelled**\n\nYour mentorship session on ${formattedDate} at ${formattedTime} (${timezone}) has been cancelled by ${cancellerName}.\n${reason ? `Reason: ${reason}\n` : ''}\nPlease visit the platform to manage your bookings: https://codewithahsan.dev/mentorship/dashboard`;
    ```
  - This covers the "mentor can ask for rescheduling" requirement without a separate reschedule state -- mentee receives a clear DM explaining it's a reschedule request and linking directly to the mentor's booking page to rebook.
