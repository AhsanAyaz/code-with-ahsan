...
Cancel handler with reason prompt (supports rescheduling flow per requirement):
```typescript
const handleCancel = async (bookingId: string) => {
  ...
  try {
    const res = await authFetch("/api/mentorship/bookings", {
      method: "PUT",
      body: JSON.stringify({ bookingId, action: "cancel", reason: reason || undefined }),
    });
    if (res.ok) {
...
The profile API endpoint `GET /api/mentorship/profile?uid={uid}` is confirmed to exist at `src/app/api/mentorship/profile/route.ts`. It returns `{ profile: { uid, displayName, photoURL, currentRole, expertise, bio, ... } }`. The profile data is stored with document ID = uid.
