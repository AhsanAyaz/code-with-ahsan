...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app/projects/[id]/page.tsx"
      to: "/api/projects/[id]"
      via: "fetch single project data"
      pattern: "api/projects/.*[^/]$"
    - from: "src/app/projects/[id]/page.tsx"
      to: "/api/projects/[id]/members"
      via: "fetch team roster"
...
<task type="auto">
  <name>Task 1: Create discovery page with ProjectCard and ProjectFilters components</name>
  <files>
    src/components/projects/ProjectCard.tsx
    src/components/projects/ProjectFilters.tsx
    src/app/projects/discover/page.tsx
  ...
  <action>
**PATH DECISION (documented):** The existing `/projects/page.tsx` is the blog's static project showcase page (renders hardcoded `projectsData` from `@/data/projectsData`). It is NOT part of the mentorship system. The mentorship project discovery lives at `/projects/discover/page.tsx` to avoid conflicting with the blog route. Do NOT modify `/projects/page.tsx`.
...
- All 3 files exist: ProjectCard.tsx, ProjectFilters.tsx, discover/page.tsx
...
- ProjectCard shows project summary with difficulty badge and tech stack
- ProjectFilters provides search, difficulty dropdown, and tech toggle badges
  </done>
</task>

<task type="auto">
  <name>Task 2: Create project detail page with ApplicationForm and TeamRoster components</name>
  <files>
    src/components/projects/ApplicationForm.tsx
    src/components/projects/TeamRoster.tsx
    src/app/projects/[id]/page.tsx
  ...
  <action>
**1. ApplicationForm.tsx** (`src/components/projects/ApplicationForm.tsx`)
- "use client" component
...
**2. TeamRoster.tsx** (`src/components/projects/TeamRoster.tsx`)
- "use client" component
...
**3. Project Detail Page** (`src/app/projects/[id]/page.tsx`)
- "use client" component with `export const dynamic = 'force-dynamic'`
- Fetch project data: `GET /api/projects/${id}` (this endpoint was added in Plan 06-02)
- Fetch team members: `GET /api/projects/${id}/members` (this endpoint was added in Plan 06-02)
- Fetch current user's application status: `GET /api/projects/${id}/applications?userId=${currentUser.uid}` (userId filter was added in Plan 06-02)
...
- All required API endpoints (GET /api/projects/[id], GET /api/projects/[id]/members, GET /api/projects/[id]/applications?userId=X) are created in Plan 06-02. This plan only consumes them.
...
- `npx tsc --noEmit` passes
- `npm run build` passes
- All 3 files exist: ApplicationForm.tsx, TeamRoster.tsx, [id]/page.tsx
