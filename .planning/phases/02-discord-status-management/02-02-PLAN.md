...
must_haves:
  truths:
    ...
    - "Admin can regenerate a Discord channel for a mentorship and receive new channel link"
    - "All changes reflect immediately in the UI without page refresh"
  artifacts:
    - path: "src/app/mentorship/admin/page.tsx"
      provides: "Inline Discord edit, status buttons, delete modal, channel URL update, regenerate channel button"
      min_lines: 1900
  key_links:
    ...
    - from: "src/app/mentorship/admin/page.tsx"
      to: "/api/mentorship/admin/sessions/regenerate-channel"
      via: "fetch POST for channel regeneration"
      pattern: "fetch.*api/mentorship/admin/sessions/regenerate-channel"
---

<objective>
Add admin UI for Discord username editing, mentorship status management, deletion, and Discord channel regeneration.

Purpose: Enable administrators to manage Discord usernames (for mentors and mentees), update Discord channel URLs, regenerate Discord channels, change mentorship status (complete/revert), and delete mentorships directly from the dashboard without database access.

Output: Enhanced admin page with inline Discord editing, status action buttons, delete confirmation modal, Discord channel URL update capability, and regenerate channel button.
</objective>
...
<task type="auto">
  <name>Task 2: Add mentorship status management, deletion, and channel regeneration UI</name>
  <files>src/app/mentorship/admin/page.tsx</files>
  <action>
Add status action buttons, delete functionality, and channel regeneration to each mentorship row:
...
**4. Create handleRegenerateChannel function:**
```typescript
const handleRegenerateChannel = async (sessionId: string) => {
  ...
  try {
    ...
    const { channelUrl } = await response.json()

    // Update local state with new channel URL
    setMentorshipData(prev => prev.map(group => ({
      ...group,
      mentorships: group.mentorships.map(m =>
        m.id === sessionId ? { ...m, discordChannelUrl: channelUrl } : m
      )
    })))

    toast.success(
      <div>
        New Discord channel created!<br/>
        <a href={channelUrl} target="_blank" rel="noopener noreferrer" className="link">
          Open channel
        </a>
      </div>
    )
  } catch (error) {
    toast.error(error instanceof Error ? error.message : "Failed to regenerate channel")
  } finally {
...
**5. Add action buttons to each mentorship row:**
...
<div className="flex gap-1 ml-auto">
  {/* Regenerate Channel button - only for active or completed */}
  {(mentorship.status === "active" || mentorship.status === "completed") && (
    <button
      className="btn btn-xs btn-ghost"
      onClick={() => handleRegenerateChannel(mentorship.id)}
      disabled={regeneratingChannel === mentorship.id}
      title="Regenerate Discord channel"
    >
      {regeneratingChannel === mentorship.id ? "..." : "🔄 Channel"}
    </button>
  )}
...
**6. Add delete confirmation modal (at end of component, before closing fragment/div):**
```tsx
...
**7. Add Discord channel URL update (inline edit):**
Add state:
```typescript
...
6. For an ACTIVE or COMPLETED mentorship:
   - Click "🔄 Channel" button - should show loading, then toast with new Discord channel link
   - Verify the channel link in the mentorship row updates
7. For any mentorship:
...
8. Test invalid transitions are prevented (API should return error for cancelled->active)
  </verify>
  <done>
...
- Active and Completed mentorships show "🔄 Channel" button that regenerates Discord channel
- Channel regeneration calls API, creates new Discord channel, updates UI with new link
- Toast shows clickable link to the new Discord channel
...
- Discord channel regeneration with new channel link
  </what-built>
  <how-to-verify>
...
**Test Discord Channel Regeneration:**
12. Find an active or completed mentorship
13. Click the "🔄 Channel" button
14. Verify loading state appears briefly
15. Verify success toast with clickable Discord channel link
16. Click the link - should open the new Discord channel
17. Verify the channel link in the mentorship row updated

**Test Mentorship Deletion:**
18. Click "Delete" on any mentorship
19. Verify confirmation modal appears with partner name
20. Click "Cancel" - modal should close, mentorship still there
21. Click "Delete" again, then confirm - mentorship should disappear

**Test Partner Discord Editing:**
22. Within a mentorship row, click on the partner's Discord username
23. Edit and save - should work same as main profile

**Cross-check in All Mentees tab:**
24. Go to "All Mentees" tab
25. Verify same functionality works from mentee perspective
  </how-to-verify>
...
6. Channel regeneration creates new Discord channel and updates session
7. Both All Mentors and All Mentees tabs have full functionality
</verification>
...
- Admin can regenerate Discord channel for active/completed mentorships
- Admin receives clickable link to new Discord channel after regeneration
