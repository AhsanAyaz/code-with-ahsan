...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/app/roadmaps/[id]/page.tsx"
      provides: "Client Component roadmap detail page"
      min_lines: 150
  key_links:
    ...
    - from: "src/app/roadmaps/[id]/page.tsx"
      ...
      via: "Client-side fetch for related mentors"
...
- Client Component roadmap detail page at `/roadmaps/[id]`
...
src/app/roadmaps/[id]/page.tsx
  ...
  <action>
Create Client Component roadmap detail page following the pattern from src/app/projects/[id]/page.tsx:
...
- Client Component with "use client" directive (matches existing pattern in src/app/projects/[id]/page.tsx)
- Use relative fetch paths: `/api/roadmaps/${roadmapId}` and `/api/mentorship/mentors?public=true` (NOT `process.env.NEXT_PUBLIC_BASE_URL` which doesn't exist)
- useEffect hook for data fetching on component mount
- useState for loading/error/data states (follows existing pattern)
...
- Back button links to `/roadmaps` catalog

**Do NOT:**
- Use Server Component async function (must be Client Component to use useState/useEffect)
- Use `process.env.NEXT_PUBLIC_BASE_URL` (doesn't exist in codebase)
- Use absolute URLs for API calls (use relative paths like existing pattern)
  </action>
  <verify>
npm run build succeeds and /roadmaps/[id] dynamic route appears in output
npx tsc --noEmit shows no type errors
Build output shows route as Client Component with "use client" directive
Page imports MarkdownRenderer and MentorCard correctly
Related mentors filtering logic uses case-insensitive matching
Fetch uses relative paths matching existing pattern in src/app/projects/[id]/page.tsx
  </verify>
  <done>
/roadmaps/[id]/page.tsx exists and renders roadmap detail page as Client Component
Page fetches roadmap content from API using relative path and displays with MarkdownRenderer
...
8. Click mentor card → navigates to mentor profile
9. Click back button → returns to /roadmaps catalog

Build verification:
- npm run build completes without errors
- /roadmaps/[id] appears as Client Component in build output (with "use client")
...
- [x] Roadmap detail page shows author attribution
...
- [x] Page uses Client Component architecture with useEffect data fetching
- [x] Fetch uses relative paths matching existing codebase pattern
</success_criteria>
