...
must_haves:
  truths:
    ...
    - "Any authenticated user sees 'Create Project' button on discover page (not just mentors)"
    - "Any authenticated user sees 'Create Project' button on My Projects 'Created' tab"
  artifacts:
    ...
    - path: "src/app/projects/discover/page.tsx"
      provides: "Create Project button visible to all authenticated users"
      contains: "Create a Project"
    - path: "src/app/projects/my/page.tsx"
      provides: "Create Project button in Created tab for all authenticated users"
...
This means:
- Creator who is NOT a member → hits "Member not found" (404) — correct
- Creator who IS a member → proceeds to leave flow — correct
- Regular member → proceeds to leave flow — correct (unchanged behavior)
...
3. Grep for "Transfer ownership" in leave/route.ts — should not be found.
...
<task type="auto">
  <name>Task 2: Update TeamRoster, project detail page, discover page, and My Projects page UI</name>
  <files>
    src/components/projects/TeamRoster.tsx
    src/app/projects/[id]/page.tsx
    src/app/projects/discover/page.tsx
...
```
When creator is also a member, show solid "Creator · Member" badge. When creator is NOT a member, show outline "Creator" badge to visually distinguish.
...
{isMember && (
```
This means any member (including creator-members) can see the Leave button. The leave endpoint handles the logic correctly — creator-members leave as members but retain owner permissions.
...
**Update `src/app/projects/discover/page.tsx`:**

1. Update the "Create a Project" button visibility logic (currently at lines 165-169). Change from mentor-only to all authenticated users:
...
This allows ANY authenticated user (not just accepted mentors) to see the Create Project button when no projects are found.

**Update `src/app/projects/my/page.tsx`:**

1. Add a "Create Project" button in the header for when the user is on the "Created" tab. Add this after the description paragraph (around line 73):
...
This replaces the existing header div (lines 69-74) with one that includes the Create Project button on the right side when viewing the "Created" tab.
  </action>
  <verify>
1. `npx tsc --noEmit` — no TypeScript compilation errors.
2. Grep TeamRoster.tsx for "nonCreatorMembers" — should appear (deduplication filter).
3. Grep page.tsx for "handleJoinProject" — should appear (join button handler).
4. Grep page.tsx for "Join Project" — should appear (button text).
5. Verify page.tsx leave button condition is `{isMember && (` not `{isMember && !isCreator && (`.
6. Grep discover/page.tsx for button condition — should be `{profile && (` not `{profile?.role === "mentor"` (any authenticated user can create).
7. Grep my/page.tsx for "Create Project" button — should appear in header when activeTab === "created".
8. Grep for project listing/discovery components to verify non-member creators are filtered from public team displays:
   - Check `src/app/projects/page.tsx` (discovery page) for how it fetches/displays team members
   - Verify that public project cards only show members from `project_members` collection (not the creator unless they're also a member)
   - Confirm that TeamRoster filtering applies to all public displays, not just the detail page
  </verify>
  <done>
- TeamRoster filters creator from members array to prevent duplication
- TeamRoster shows "Creator · Member" dual-role badge when creator is also a member
...
- Discovery page shows "Create a Project" button to all authenticated users (not just mentors)
- My Projects page shows "Create Project" button in header when on "Created" tab
- All TypeScript compiles without errors
- Public discovery pages only show members from project_members collection (non-member creators are not displayed in public team rosters)
  </done>
</task>
...
<verification>
1. `npx tsc --noEmit` — no compilation errors across entire project
...
6. Verify public discovery filtering: non-member creators not shown in public project directories
</verification>
...
- TeamRoster shows creator with correct badge ("Creator" or "Creator · Member") and never duplicates creator in member list
- Public project directories only show members from project_members collection (non-member creators are filtered out)
