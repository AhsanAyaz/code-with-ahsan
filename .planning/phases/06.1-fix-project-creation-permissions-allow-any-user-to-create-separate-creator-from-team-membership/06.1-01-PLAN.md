...
8. Update the inline comment on line 87 from "Check permission: only accepted mentors can create projects" to "Check permission: any authenticated user can create projects".
  </action>
  <verify>
...
- API error message updated to reflect new permission model
  </done>
</task>
...
Note: Although roadmap creation remains mentor-only, if a roadmap somehow has a non-mentor owner, they should still be able to edit it. The `canOwnerOrAdminAccess` change is consistent across all resources.
  ...
  <verify>
Run `npx vitest run src/__tests__/permissions.test.ts` — ALL tests must pass. Expected count should increase from 50 to ~54 tests (4 new test cases added).
  </verify>
  <done>
...
- Test count increased to reflect new test cases
  </done>
</task>
