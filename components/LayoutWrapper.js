...
const LayoutWrapper = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const onClick = (e) => {
      ...
      if (!moreRef.current.contains(e.target) && !moreBtnRef.current.contains(e.target)) {
        setIsMoreOpen(false)
      }
    }
              ...
              {moreLinks.length > 0 && (
                <div className="ml-2 relative" ref={moreRef}>
                  <button
                    ...
                    className="p-1 font-bold text-gray-900 sm:p-4 dark:text-gray-100 inline-flex items-center gap-1 hover:underline underline-offset-8"
                  >
                    More
                    <svg
                  ...
                  <div
                    role="menu"
                    className={`absolute right-0 mt-2 min-w-[10rem] rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg py-1 z-50 ${
                  ...
                  >
                    {moreLinks.map((link) => (
                      <Link
                        key={link.title}
                        href={link.href}
                        className="block w-full text-left px-4 py-2 text-sm text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700"
